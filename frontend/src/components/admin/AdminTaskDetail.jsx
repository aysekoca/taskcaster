...
import {
  adminGetUsersFetch,
  resetAdminGetUsersState,
} from "../../redux/slices/admin/user/adminGetUsersSlice";
...
// getFileIcon için gerekli olanlar:
...
const AdminTaskDetail = () => {
  ...
  useEffect(() => {
    if (adminGetUsersState.data) {
      dispatch(setAdminUsersState(adminGetUsersState.data.data));
      dispatch(resetAdminGetUsersState());
    }
  }, [adminGetUsersState]);

  useEffect(() => {
    if (!categoriesState?.length) dispatch(categoriesFetch());
    if (!adminUsersState?.length) dispatch(adminGetUsersFetch());
    fetchTaskDetails();
  }, [id]);
  ...
  const fetchTaskDetails = async () => {
    setIsLoading(true);
    try {
      const res = await api.get(`admin/task/details/${id}`);
...
const Detail = ({ task, setTask, setIsEditing, categories }) => {
  ...
  const onStatusChange = async (task, val) => {
    ...
    try {
      await api.patch(`admin/task/${task.id}`, {
        ...task,
      ...
      <Grid size={{ xs: 12, lg: 8 }} sx={{ display: "flex", minWidth: 0 }}>
        <Paper variant="outlined" sx={{ p: 3, borderRadius: 4, flexGrow: 1 }}>
          <Box sx={{ display: "flex", justifyContent: "space-between", mb: 3 }}>
            ...
            <Stack direction={{xs:"column",md:"row"}} sx={{mb:4}} alignItems="center" spacing={1}>
              <Button
              fullWidth
                variant="outlined"
                color="error"
            ...
            >
              ...
              <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                ...
                <Box>
                  ...
                  >
                    <EmailIcon sx={{ fontSize: 14 }} /> {task.userEmail}
                  </Typography>
                </Box>
              </Box>
            </Paper>
            ...
            {task.files?.map((file) => (
              ...
              >
                ...
                <Box sx={{ flexGrow: 1, minWidth: 0 }}>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    Upload Date:{" "}
                    {dayjs(file.uploadDate).format("YYYY/MM/DD HH:mm")}
                  </Typography>
                </Box>
...
const Edit = ({ task, setTask, setIsEditing, categories, users }) => {
  const dispatch = useDispatch();
  const { showConfirm } = useConfirmation();
  ...
  const handleSave = async (e) => {
    ...
    try {
      ...
      await api.patch(`admin/task/${task.id}`, { ...updatedFormData });
      ...
      if (newFiles.length > 0) {
        const uploadPromises = newFiles.map(async (file) => {
          ...
          try {
            return await api.post(`admin/file/${task.id}`, data, {
              onUploadProgress: (pe) =>
        ...
        if (failedFiles.length > 0) {
          const errorFilesStr = failedFiles
            .map((f) => f.fileName)
            .join("<br/>");
          alertify.error(
            `Some files could not be uploaded:<br/>${errorFilesStr}`,
            5
          );
        }
      }

      const updated = await api.get(`admin/task/details/${task.id}`);
      ...
      dispatch(
        setSnackBarState({
          message: normalizeGoError(err.response?.data?.message || err.message),
          status: "error",
          isOpen: true,
        })
      );
  ...
  const handleChangeF = (incomingFiles) => {
    ...
    incomingFiles.forEach((file) => {
      const isDuplicate =
        updatedFilesArr.findIndex((f) => f.name === file.name) !== -1;
        ...
        <Paper variant="outlined" sx={{ p: 3, borderRadius: 4, flexGrow: 1 }}>
          ...
          <Stack spacing={3}>
            <Autocomplete
              ...
              onChange={(_, v) => setFormData({ ...formData, userId: v?.id })}
              renderInput={(p) => (
                <TextField
                  {...p}
                  label="Reassign Task (Email)"
                  InputProps={{
                    ...p.InputProps,
                    startAdornment: (
        ...
        >
          <Stack spacing={3} sx={{ height: "100%", p: 3 }}>
            ...
            <Box>
              ...
              <MuiFileInput
                value={newFiles}
                onChange={(files) => handleChangeF(files)}
                multiple
                hideSizeText
            ...
            {/* --- 3. NEW FILES LIST WITH PROGRESS GRADIENT --- */}
            <Stack
              spacing={1}
                ...
                return (
                  <Box
                    ...
                    sx={{
                      ...
                      borderRadius: 2,
                      border: "1px solid",
                      borderColor: isUploading ? "primary.main" : "#e3f2fd",
                      position: "relative",
                      overflow: "hidden",
